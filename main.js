// Version: Space Weather Adapter Hauptlogik (Fix V17 - Entfernen des Proton-Flux-Filters)
"use strict";
...
class SpaceWeatherAdapter extends utils.Adapter {
    ...
    async onReady() {
        ...
        // Objekterstellung forciert, um Race Condition zu beheben (Fix V3)
    ...
    async ensureObjects() {
        ...
        for (const [id, meta] of Object.entries(REQUIRED_CHANNELS)) {
            // Kanäle können mit setObjectsNotExistsAsync erstellt werden, da sie sich selten ändern
        ...
        for (const [id, meta] of Object.entries(REQUIRED_STATES)) {
            // HIER WIRD setObjectsAsync verwendet, um das Objekt forciert zu erstellen/aktualisieren 
            // und es sofort im ioBroker-Cache sichtbar zu machen, um die Race Condition zu verhindern.
    ...
    async deleteUnusedObjects() {
        ...
        // Set of all required IDs (prefix-less)
    ...
    async getLatestValueFromFeed(url, valueIndex) {
        try {
            ...
            if (!Array.isArray(response.data) || response.data.length < 2) {
                this.log.warn(`API feed from ${url} is empty or invalid (Status ${response.status}).`);
                return null;
            }
            ...
            for (let i = response.data.length - 1; i > 0; i--) { 
                ...
                // FIX V15: Robuste Typenprüfung: Metadaten-Objekte oder Nullwerte überspringen
                if (!Array.isArray(current)) {
                    const rawDataString = (typeof current === 'object' && current !== null) ? JSON.stringify(current) : String(current);
                    // Dies ist das Verhalten, das wir für X-Ray/Proton ABSTELLEN mussten. Es bleibt für ARRAY-OF-ARRAY-Feeds korrekt.
                    this.log.debug(`[FEED REJECTED] Entry ${i} skipped. Reason: Not an array/Skipping metadata. Raw Data (Full): ${rawDataString}`);
                    continue; 
                }
                ...
                if (current.length > valueIndex) {
                    ...
                    if (valueLower === "" || valueLower === "nan" || valueLower === "-99999.00" || valueLower.includes("-1e05")) {
                        if (valueLower.includes("-1e05") || valueLower.includes("-99999") || valueLower === "nan") {
                             this.log.debug(`[FEED SKIP] Datapoint ${valueIndex} in entry ${i} (${timeTag}) skipped. Reason: Known NOAA missing/error code. Raw Data: ${current.join('|')}`);
                            continue;
                        }
                    }
                    ...
                    if (isFinite(parsedValue)) {
                        ...
                        this.log.debug(`[FEED REJECTED] Datapoint ${valueIndex} in entry ${i} (${timeTag}) rejected. Reason: Could not be interpreted as a finite number. Raw Data (Full): ${current.join('|')}`);
                    }
                } else {
                    this.log.debug(`[FEED REJECTED] Entry ${i} rejected. Reason: Array length (${current.length}) is too short for required index ${valueIndex}. Raw Data (Full): ${current.join('|')}`);
                }
            }
            ...
            if (error.response && error.response.status === 404) {
                this.log.error(`Request failed with status code 404 for URL: ${url}. This API endpoint may be obsolete.`);
            }
            return null;
        }
    }
    ...
    /**
     * FIX V16: Fetches the latest value from a NOAA object feed.
    ...
    async getLatestValueFromObjectFeed(url, valueKey, filterKey = null, filterValue = null) {
        try {
            this.log.debug(`Attempting to fetch data (OBJECT-FEED) from URL: ${url}.`);
            const response = await axios.get(url);
            this.log.debug(`API call successful. HTTP Status: ${response.status} for ${url}`);
            ...
            // Iterate backwards, starting from the last entry
            for (let i = response.data.length - 1; i >= 0; i--) { 
                const current = response.data[i]; 
                
                // 1. Ensure it's a valid object with time and value key
                if (typeof current !== 'object' || current === null || current[valueKey] === undefined || !current.time_tag) {
                     this.log.debug(`[OBJECT-FEED REJECTED] Entry ${i} skipped. Reason: Missing required keys (time_tag or ${valueKey}). Raw Data: ${JSON.stringify(current)}`);
                    continue; 
                }
                
                // 2. Apply optional filter
                if (filterKey && current[filterKey] !== filterValue) {
                    // FIX V17: Zusätzliche Prüfung, ob der Filter-Key überhaupt existiert, falls nötig.
                    const filterValueInObject = current[filterKey] !== undefined ? current[filterKey] : "undefined";
                     this.log.debug(`[OBJECT-FEED SKIP] Entry ${i} skipped. Reason: Filter mismatch (${filterKey}=${filterValueInObject}).`);
                    continue;
                }
                ...
                if (valueLower === "" || valueLower === "nan" || valueLower === "-99999.00" || valueLower.includes("-1e05")) {
                    this.log.debug(`[OBJECT-FEED SKIP] Value in entry ${i} (${current.time_tag}) skipped. Reason: Known NOAA missing/error code.`);
                    continue;
                }
                ...
                if (isFinite(parsedValue)) {
                    ...
                    this.log.debug(`[OBJECT-FEED REJECTED] Value in entry ${i} (${current.time_tag}) rejected. Reason: Could not be interpreted as a finite number. Raw Data: ${JSON.stringify(current)}`);
                }
            }
            this.log.warn(`API feed from ${url} contained no single valid value for key '${valueKey}' (Filter: ${filterValue}).`);
            return null;
    ...
    async getLatestPlasmaData(url) {
        try {
            ...
            for (let i = response.data.length - 1; i > 0; i--) { 
                ...
                // Robuste Typenprüfung: Metadaten-Objekte oder Nullwerte überspringen
                if (!Array.isArray(current)) {
                    const rawDataString = (typeof current === 'object' && current !== null) ? JSON.stringify(current) : String(current);
                    this.log.debug(`[PLASMA REJECTED] Entry ${i} skipped. Reason: Not an array/Skipping metadata. Raw Data: ${rawDataString}`);
                    continue; 
                }
                ...
                if (current.length > N_INDEX) {
                   ...
                   const nRaw = String(current[N_INDEX]).trim().toLowerCase();
                    ...
                    if (timeTag && vRaw && nRaw) {
                        // Check for missing/error values in V and N
    ...
    /**
     * Fetches the latest observed sunspot number from the long-term data file.
     * @returns {Promise<number | null>} The latest observed sunspot number (SSN).
     */
    async fetchSunspotNumber() {
        // Monatlicher Index-Feed. Enthält Objekte mit keys wie "time-tag", "observed_swpc_ssn"
        const url = "https://services.swpc.noaa.gov/json/solar-cycle/observed-solar-cycle-indices.json"; 
        this.log.debug(`Attempting to fetch Sunspot Number from (Monthly Index Feed): ${url}`);
        ...
        try {
            ...
            // Definiere die benötigten Schlüssel für die JSON-Objekte
            const SSN_KEY = "observed_swpc_ssn"; 
            
            // Iteriere rückwärts, beginnend mit dem aktuellsten Eintrag
            for (let i = response.data.length - 1; i >= 0; i--) { 
                const current = response.data[i]; 
                
                // Prüfen, ob es ein gültiges Objekt ist und den benötigten Schlüssel enthält
                if (typeof current === 'object' && current !== null && current[SSN_KEY] !== undefined) {
                    ...
                    // Nur Werte, die >= 0 sind, werden als gültig akzeptiert.
                    if (isFinite(ssn) && ssn >= 0) {
                        this.log.info(`Latest valid Sunspot Number found (${current["time-tag"]}): ${ssn}`);
                        return ssn;
                    } else {
                        continue;
                    }
            ...
            this.log.warn(`No valid Sunspot Number (observed_ssn >= 0) found in the feed: ${url}.`);
            return null;
    ...
    /**
     * Fetches the latest Proton Flux data. Uses the new OBJECT-FEED parser.
     * @returns {Promise<{value: number, time_tag: string} | null>}
     */
    async fetchProtonFlux() {
        // Integral Electron and Proton Flux (PCE) URLs. These now return OBJECT arrays.
        const protonUrls = [
            // Robuster 7-Tage-Fallback (Secondary GOES)
            "https://services.swpc.noaa.gov/json/goes/secondary/integral-protons-7-day.json",   
            // Robuster 7-Tage-Fallback (Primary GOES)
            "https://services.swpc.noaa.gov/json/goes/primary/integral-protons-7-day.json",    
            // Primäre, meist aktuelle 6-Stunden-Daten
        ...
        for (const url of protonUrls) {
            attempt++;
            this.log.debug(`Attempting Proton Flux URL ${attempt}: ${url}`);
            
            // FIX V17: Entferne den Filter "channel", da er das Parsing blockiert.
            // Der Feed selbst ist für Integral Protons spezifisch.
            ...
            if (protonData) {
                ...
                // Nur warnen, wenn es noch Fallbacks gibt
                if (attempt < protonUrls.length) {
                    this.log.warn(`Proton Flux URL ${attempt} failed. Trying next fallback.`);
                }
            }
        }
    ...
    /**
     * Fetches the latest X-Ray Flux data with a fallback logic. Uses the new OBJECT-FEED parser.
     * @returns {Promise<{value: number, time_tag: string} | null>}
     */
    async fetchXRayFlux() {
        // GOES X-Ray Flux URLs. These now return OBJECT arrays.
        const xrayUrls = [
            // Priorisiert 7-Tage-Feeds
            "https://services.swpc.noaa.gov/json/goes/primary/xrays-7-day.json",      
            "https://services.swpc.noaa.gov/json/goes/secondary/xrays-7-day.json",    
            // Fallback 6-Stunden-Feeds
        ...
        for (const url of xrayUrls) {
            ...
            // FIX V16: Nutze den Object-Feed-Parser mit Filter auf Energie-Kanal
    ...
    async fetchData(apiKey) {
        ...
        for (const url of kpUrls) {
            ...
            // Kp-Index ist Array-of-Array, daher wird getLatestValueFromFeed verwendet. Index 1 ist Kp.
        ...
        const kpForecastData = await this.getLatestValueFromFeed(kpForecastUrl, 1); // Index 1 ist der Kp-Forecast
        if (kpForecastData) {
        ...
        if (protonData) {
            // Der P>10 MeV Wert ist nun direkt die Value aus dem Object-Feed-Parser
            fetchedValues.solarwind_proton_flux = protonData.value;
            updateLatestTimeTag(protonData.time_tag);
            this.log.info(`Proton Flux: ${protonData.value} pfu`);
        } else {
            this.log.warn("Proton Flux Data could not be updated.");
        ...
        for (const url of f107Urls) {
            ...
            f107Data = await this.getLatestValueFromFeed(url, 1); // Index 1 ist der Flux
            if (f107Data) {
        ...
        for (const url of dstUrls) {
            ...
            dstData = await this.getLatestValueFromFeed(url, 1); // Index 1 ist der Dst
            if (dstData) {
        ...
        if (xrayData) {
            ...
            this.log.error("GOES X-Ray Flux could not be updated using any available feeds.");
        }
        ...
        // --- 11. Sunspot Number ---
        const sunspotNumber = await this.fetchSunspotNumber(); // Logging entfernt, aber Logik behalten
        if (sunspotNumber !== null) {
            fetchedValues.solar_activity_sunspot_number = sunspotNumber;
            this.log.info(`Sunspot Number updated: ${sunspotNumber}`);
        } else {
            this.log.warn("Sunspot Number could not be updated.");
        ...
        // Setzt den letzten Update-Zeitstempel
        if (latestTimeTag) {
            await this.setStateAsync("last_update", { val: latestTimeTag, ack: true });
        } else {
            this.log.warn("No timestamp found from any of the observed feeds. States will be updated with 0/empty values.");
        }
